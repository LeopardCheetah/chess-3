class ChessGame:
    ...
    def generate_white_candidate_moves(self):
        ...
        for square in self.white_piece_locations:
            ...
            if piece_type == 'P':
                ...
                if rank == '7':
                    ...
                    if self.game_board.get_piece_color_atsq(file + '8') is None:
                ...
                if self.game_board.get_piece_color_atsq(file + str(int(rank) + 1)) is None:
                    # pawn is free to move up (technically -- if not pinned)
                    candidate_moves.append((file + rank, file + str(int(rank) + 1)))

                    if rank == '2' and self.game_board.get_piece_color_atsq(file + '4') is None:
        ...
        for pair in candidate_moves:
            ...
            # reset position
            if final_square_piece_type not in [-1, None]:
                # piece exists
                self.game_board.move_piece(pair[1], pair[0])
                if final_square_piece_color is None and final_square_piece_type is None:
                    continue
        ...
        if self.white_kingside_castling:
            ...
            if self.game_board.get_piece_type_atsq('f1') is None and self.game_board.get_piece_type_atsq('g1') is None:
        ...
        if self.white_queenside_castling:
            if self.game_board.get_piece_type_atsq('b1') is None and self.game_board.get_piece_type_atsq('c1') is None and self.game_board.get_piece_type_atsq('d1') is None:
    ...
    def generate_black_candidate_moves(self):
        ...
        for square in self.black_piece_locations:
            ...
            if piece_type == 'P':
                if rank == '2':
                    ...
                    if self.game_board.get_piece_color_atsq(file + '1') is None:
                ...
                if self.game_board.get_piece_color_atsq(file + str(int(rank) - 1)) is None:
                    ...
                    if rank == '7' and self.game_board.get_piece_color_atsq(file + '5') is None:
                        # we can move 2 squares yippee
        ...
        if self.black_kingside_castling:
            if self.game_board.get_piece_type_atsq('f8') is None and self.game_board.get_piece_type_atsq('g8') is None:
                # no pieces on f8, g8
        ...
        if self.black_queenside_castling:
            if self.game_board.get_piece_type_atsq('b8') is None and self.game_board.get_piece_type_atsq('c8') is None and self.game_board.get_piece_type_atsq('d8') is None:
